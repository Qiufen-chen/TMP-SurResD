...
class mydataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        width, height, depth = label_data.shape[0], label_data.shape[1], 1
        label_data = label_data.reshape([depth, width, height])

        label_data = torch.from_numpy(label_data)

        return feature, label_data
...
def main():
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            th.save(model.state_dict(), "/lustre/home/qfchen/ContactMap/SurContact/model/" + "Before_onehot_pair_" + str(epoch))
