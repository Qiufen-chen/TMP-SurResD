...
def pcc(y_pred,y_true):
    ...
    # print(y_pred[:100])
    cc = my_pcc(y_pred, y_true, y_true.shape[0])
    # print(cc)
    return cc
...
def main(n):
    ...
    optimizer = th.optim.Adamax(model.parameters(), lr=0.00001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
    scheduler = th.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=0.1, patience=5, threshold=0.00001, threshold_mode='rel', cooldown=0,
        min_lr=0.0000001, eps=1e-08, verbose=False)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for index, (x, y_true) in enumerate(val_loader):
                ...
                r2_1 = r2(y_true, y_pred)
                cc_1 = pcc(y_true, y_pred)
        ...
        if epoch % 10 == 0:
            th.save(model.state_dict(), "/lustre/home/qfchen/ContactMap/TMContact/model/" + "mask_10_hmm_ccmpred_" + str(epoch))
